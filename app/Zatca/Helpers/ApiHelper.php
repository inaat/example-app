...
class ApiHelper {
    ...
    private static function sendRequestWithRetry($url, $headers, $payload, $auth = null, $method = 'POST', $retries = 3, $backoffFactor = 2) {
        for ($attempt = 0; $attempt < $retries; $attempt++) {
            ...
            if (curl_errno($ch)) {
                ...
                error_log("ZATCA API Error (Attempt " . ($attempt + 1) . "/$retries): [$errno] $error_msg");

                if ($attempt < $retries - 1) {
                    $sleepTime = $backoffFactor ** $attempt;
                    error_log("Retrying in $sleepTime seconds...");
                    sleep($sleepTime);
                    continue;
                } else {
                    throw new Exception("cURL error after $retries retries: [$errno] $error_msg");
                }
            }
            ...
            if ($httpCode === 200 || $httpCode === 202) {
                ...
                error_log("ZATCA API HTTP Error (Attempt " . ($attempt + 1) . "/$retries): $httpCode - $response");
    ...
    public static function checkServiceStatus($environmentType = 'NonProduction') {
        ...
        $url = "https://gw-fatoora.zatca.gov.sa/e-invoicing/$apipath/compliance";
        ...
        return [
            'status' => $errno === 0 && $httpCode < 500,
            'http_code' => $httpCode,
            'error' => $error ?: null,
            'message' => $errno === 0 
                ? ($httpCode < 500 ? 'Service appears operational' : "Service unavailable (HTTP $httpCode)")
