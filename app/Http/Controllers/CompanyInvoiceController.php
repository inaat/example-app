...
class CompanyInvoiceController extends Controller
{
    ...
    {
        $invoices = CompanyInvoice::with(['company', 'returns', 'debits'])
                               ->where('invoice_type', '388') // Only regular invoices (388)
                               ->orderBy('created_at', 'desc')
                               ->paginate(15);
        return view('zatca.company.invoices.index', compact('invoices'));
    }
    ...
    public function generateXML(CompanyInvoice $invoice)
    {
        ...
        try {
            ...
            // Set supplier party using company data
            $company = $invoice->company;
            
            $sellerAddress = [
                'streetName' => $company->street_name ?? 'الامير سلطان | Prince Sultan',
                'buildingNumber' => str_pad($company->building_number ?? '2322', 4, '0', STR_PAD_LEFT),
                'citySubdivisionName' => $company->sub_division_name ?? 'المربع | Al-Murabba',
                'cityName' => $company->city_name ?? 'الرياض | Riyadh',
                'postalZone' => $company->postal_number ?? '23333',
            ...
            \Log::info('Company VAT Debug', [
                'company_serial' => $company->serial_number,
            ...
            $builder->setSupplierParty(
                $company->crn ?? '1010010000',
                $sellerVat,
                $invoice->seller_info['name'] ?? $company->organization_name ?? 'Test Company LTD',
                $sellerAddress
            );
            ...
            $builder->setCustomerParty(
                $isSimplified ? null : '1010010000',
                $invoice->buyer_info['vat_number'] ?? '399999999800003',
                $invoice->buyer_info['name'] ?? 'Customer LTD',
                $buyerAddress
            );

            // Add delivery date for standard invoices (BR-KSA-15)
            if (!$isSimplified) {
    ...
    public function signInvoice(CompanyInvoice $invoice)
    {
        ...
        try {
            ...
            $x509CertificateContent = base64_decode(
                $company->pcsid_binarySecurityToken ?: $company->ccsid_binarySecurityToken
            );
    ...
    public function submitToZatca(CompanyInvoice $invoice)
    {
        ...
        try {
            ...
            // Prepare company info array for API call
            $companyInfoArray = $company->toArray();
            ...
            if ($isSimplified) {
                $response = ApiHelper::invoiceReporting($companyInfoArray, $jsonPayload);
                $statusKey = 'reportingStatus';
            } else {
                $response = ApiHelper::invoiceClearance($companyInfoArray, $jsonPayload);
            ...
            if ($isSuccess) {
                $company->updateLastInvoiceHash($invoice->current_hash);
            }
            ...
            Log::error('Company ZATCA Submission Failed', [
                'error' => $e->getMessage(),
    ...
    public function generateQRCode(CompanyInvoice $invoice)
    {
        try {
            if (!$invoice->signed_xml) {
                return response()->json([
    ...
    public function destroy(CompanyInvoice $invoice)
    {
        ...
        $invoice->delete();
        return redirect()->route('zatca.company.invoices.index')
